...
contains
    ...
    subroutine xml_write_pre_enlistment(filePath, eList, Section, curriculumFilter)
        ...
        call xml_open_file(unitNo, XML_ROOT_ENLISTMENT, fileName, eof)
        ...
        do std=1,NumStudents
            ...
            call xml_write_integer  (unitNo, indent1, 'UnitsEarned', eList(std)%UnitsEarned)
            call xml_write_integer  (unitNo, indent1, 'StdYear', eList(std)%StdYear)
            call xml_write_integer  (unitNo, indent1, 'StdClassification', eList(std)%StdClassification)
            call xml_write_integer  (unitNo, indent1, 'AllowedLoad', eList(std)%AllowedLoad)
            call xml_write_integer  (unitNo, indent1, 'StdPriority', eList(std)%StdPriority)
            call xml_write_integer  (unitNo, indent1, 'NPriority', eList(std)%NPriority)
            if (eList(std)%NAlternates/=0) &
            call xml_write_integer  (unitNo, indent1, 'NAlternates', eList(std)%NAlternates)
            if (eList(std)%NCurrent/=0) &
            call xml_write_integer  (unitNo, indent1, 'NCurrent', eList(std)%NCurrent)

            do i=1,lenRecord
                sect = eList(std)%Section(i)
                if (sect>0) then
                    call xml_write_character(unitNo, indent1, 'Enlisted', Section(sect)%ClassId)
                elseif (eList(std)%Subject(i)>0) then
                    if (eList(std)%Contrib(i)>0.0) then
                        call xml_write_character(unitNo, indent1, 'Predicted', &
                            trim(Subject(eList(std)%Subject(i))%Name)//COMMA//ftoa(eList(std)%Contrib(i)) )
                    else
                        call xml_write_character(unitNo, indent1, 'Allowed', Subject(eList(std)%Subject(i))%Name)
                    end if
                end if
            end do
            ! student info end
        ...
        ! close file
        call xml_close_file(unitNo, XML_ROOT_ENLISTMENT)
    ...
    subroutine custom_read_pre_enlistment(filePath, actionIndex, skipCurrent, NumSections, Section, cList, numEntries, ier)
        ...
        do
            ...
            call GetEnlistment(unitNo, NumSections, Section, line, cList(0))
            if (std==0) then
