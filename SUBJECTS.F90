...
module SUBJECTS
    ...
    real, dimension (MAX_ALL_DUMMY_SUBJECTS:MAX_ALL_SUBJECTS,1:3) :: FailRate
    ...
    integer, private :: eof, ndels, pos(50)
...
!        call xml_open_file(unitETC, XML_ROOT_SUBJECTS, fileName, i)
!
...
!            end do
!
!            call xml_write_character(unitETC, indent0, 'Subject')
!            call xml_write_character(unitETC, indent1, 'Code', Subject(subj)%Name)
!            if (Subject(subj)%Units>0.0) call xml_write_character(unitETC, indent1, 'DisplayUnits', ftoa(Subject(subj)%Units,2))
!            if (Subject(subj)%Tuition/=0.0) call xml_write_float(unitETC, indent1, 'Tuition', Subject(subj)%Tuition,2)
!            if (Subject(subj)%LabFee/=0.0) call xml_write_float(unitETC, indent1, 'LabFee', Subject(subj)%LabFee,2)
!            if (Subject(subj)%LectLoad/=0.0) call xml_write_float(unitETC, indent1, 'LectLoad', Subject(subj)%LectLoad,2)
!            if (Subject(subj)%LabLoad/=0.0) call xml_write_float(unitETC, indent1, 'LabLoad', Subject(subj)%LabLoad,2)
!            if (trim(mesg1)/='NONE') call xml_write_character(unitETC, indent1, 'Prerequisite', mesg1)
!            if (trim(mesg2)/='NONE') call xml_write_character(unitETC, indent1, 'Corequisite', mesg2)
!            if (trim(mesg3)/='NONE') call xml_write_character(unitETC, indent1, 'ConcurrentWith', mesg3)
!            if (trim(mesg4)/='NONE') call xml_write_character(unitETC, indent1, 'ConcurrentPrerequisite', mesg4)
!            call xml_write_character(unitETC, indent0, '/Subject')
!
!        end do
!
!        call xml_close_file(unitETC, XML_ROOT_SUBJECTS)
...
!        end if
    ...
    subroutine xml_read_subjects(path, errNo)
        ...
        !write(unitLOG,*) 'subject codes=', NumSubjects
        ...
        call xml_open_file(unitETC, XML_ROOT_SUBJECTS, fileName, eof, forReading)
        do
...
!    end subroutine xml_read_subjects_other
...
#if defined UPLB
                if (k==1) then
                    Failrate(cdx,3) = (1.0*i)/max(1,j)
                elseif (k==2) then
                    Failrate(cdx,1) = (1.0*i)/max(1,j)
                else
                    Failrate(cdx,2) = (1.0*i)/max(1,j)
                end if
#else
    ...
    subroutine xml_write_failrates(path, GrandTotal)
        ...
        character(len=*), intent(in) :: path
        integer, intent (in), dimension(MAX_ALL_SUBJECTS,2,1:3) :: GrandTotal
        ...
        '        Generated by '//PROGNAME//VERSION//' on '//currentDate(1:4)// &
                    FSLASH//currentDate(5:6)//FSLASH//currentDate(7:8), &
        '        Failrate - Subject,#(Fail 1st),#(Enrolled 1st),#(Fail 2nd),#(Enrolled 2nd),#(Fail Sum.),#(Enrolled Sum.)', &
        '                  #(Pass) = #(Grade>=75) + #(Grade<=3.0); #(Fail) = not Pass', &
        '    </comment>'

        do cdx=1,NumSubjects
            if (sum(GrandTotal(cdx,1:2,1:3))==0) cycle
            call xml_write_character(unitXML, indent0, 'Failrate', &
                ...
                trim(itoa(sum(GrandTotal(cdx,1:2,2))))//COMMA// &
                trim(itoa(GrandTotal(cdx,2,3)))//COMMA// &
                trim(itoa(sum(GrandTotal(cdx,1:2,3)))) )
        end do
