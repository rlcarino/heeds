...
contains
    ...
    subroutine enlistment_grades (device, NumSections, Section)
        ...
        integer :: gdx, ldx, n_count, tdx, std, ierr, sect, ncol, crse, idx_select
        ...
        do tdx=1,NumStudents
            ...
            do ncol=1,Preenlisted(std)%lenSubject
                ...
                if (targetSection == sect) then
                    if (trim(tAction)=='Submit') then
                        call cgi_get_named_string(QUERY_STRING, trim(Student(std)%StdNo), tGrade, ierr)
                        if (ierr==0) then
                            isDirtyFORM5 = .true.
                            Preenlisted(std)%Grade(ncol) = index_to_grade(tGrade)
                        end if
                    end if
        ...
        call list_sections_to_edit(device, Section, ncol-n_count, tArray(n_count+1:), &
            fnTeacherClasses, SPACE, 'Other classes', .true., &
            '<b>GRADESHEET for '//trim(Subject(crse)%Name)//' - '//trim(header)//'</b><br><br>')

        if (n_count == 0) then
            write(device,AFORMAT) '<br>(No students in this section?)'
        else
            call make_form_start(device, fnGradeSheet, tClassId)
            write(device,AFORMAT) '<br><table border="0" width="70%">', &
                begintr//thalignleft//'#'//endth//thalignleft//'Student No.'//endth, &
                thalignleft//'Student Name'//endth//thalignleft//'Curriculum'//endth, &
                thalignleft//'Grade'//endth//endtr
...
!                write(device,AFORMAT) '<option value="'//trim(tGrade)//'" '//trim(selected(idx_select))//'> '//tGrade
            ...
            write(device,AFORMAT) begintr//tdnbspendtd//tdnbspendtd//tdnbspendtd//tdnbspendtd//begintd, &
                '<input type="submit" name="action" value="Submit">'//endtd//endtr//'</table></form>'
        end if
    ...
    subroutine enlistment_edit (device, NumSections, Section, NumBlocks, Block)
            ...
            case ('Switch')
                ...
                do fdx=1,Advice%lenSubject
                    ...
                    do mdx=1,Preenlisted(targetStudent)%lenSubject
                        if (Preenlisted(targetStudent)%Subject(mdx)/=crse) cycle
                        if (Preenlisted(targetStudent)%Section(mdx)>0) then
                            call html_comment('Retained section '// &
                                Section(Preenlisted(targetStudent)%Section(mdx))%Classid)
                        else
        ...
        call timetable_meetings_of_student(NumSections, Section, targetStudent, Preenlisted, 0, tLen1, tArray, TimeTable, &
            conflicted)
        !!
        ...
        call list_sections_to_edit(device, Section, tLen1, tArray, fnChangeMatriculation, tStdNo, 'Del', allowed_to_edit, &
            ...
            A1=tStdNo, pre='<small>(', post=')</small>', alt=SPACE)) )
        ...
        if (tLen2>0) then
            mdx = 0
            write(device,AFORMAT) '<br><b>Alternate subjects</b>:'
            do fdx=1,Preenlisted(targetStudent)%lenSubject
                if (Preenlisted(targetStudent)%Section(fdx)==0) then
                    mdx = mdx+1
                    tSubject = Subject(Preenlisted(targetStudent)%Subject(fdx))%Name
                    write(device,AFORMAT) '<br>'//trim(itoa(mdx))//'). <a href="#'// &
                    trim(tSubject)//'">'//trim(tSubject)//'</a> - '// &
                    trim(Subject(Preenlisted(targetStudent)%Subject(fdx))%Title)
                end if
            end do
            ...
            do fdx=1,Preenlisted(targetStudent)%lenSubject
                ...
                if (tLen2>0) then ! there are sections that can be added
                    ...
                    '<a name="'//trim(tSubject)//'"></a><br><hr>'// &
                    green//'<b>AVAILABLE sections'//black//' in '//trim(tSubject)// &
                    ...
                    call list_sections_to_edit(device, Section, tLen2, tArray(tLen1+1), fnChangeMatriculation, tStdNo, 'Prerog', &
                        allowed_to_edit, '<a name="'//trim(tSubject)//'"></a> <br> <hr>'// &
                        red//'<b>"TEACHER''S PREROGATIVE" sections'//black//' in '// &
    ...
    subroutine enlistment_printable  (device, NumSections, Section)
        ...
        call timetable_meetings_of_student(NumSections, Section, targetStudent, Preenlisted, 0, tLen1, tArray, &
        ...
        do blk=1,NumBlocks
            ...
            do bdx=1,Block(blk)%NumClasses
                ...
                if (sect/=0) then
                    ...
                    if (seats>0) then
                        write(device,AFORMAT) '<b>'//green//trim(Section(sect)%ClassId)//' ('// &
                        trim(itoa(seats))//') '//black//'</b> /'//nbsp
                    else
                        write(device,AFORMAT) '<b>'//green//trim(Section(sect)%ClassId)//black//red//' ('// &
                        trim(itoa(seats))//') '//black//'</b> /'//nbsp
                    end if
                else
                    write(device,AFORMAT) '<b>'//red//trim(Subject(crse)%Name)//black//'</b> /'//nbsp
                end if
            ...
            write(device,AFORMAT) trim(make_href(fnChangeMatriculation, 'Enlist', A1=tStdNo, A2='Block', &
                A3=Block(blk)%BlockID, pre=nbsp, post=endtd//endtr, alt=SPACE))
        end do
        if (n_blks==0) then
            write(device,AFORMAT) begintr//'<td colspan="3">No suitable blocks for this student?'//endtd//endtr
        end if
        write(device,AFORMAT) '</table>', &
        '<br><i>NOTES</i> : A <b>'//green//'Class ID (seats)'//black//'</b> is open. A <b>'//green//'Class ID '//black, &
        red//'(0)'//black//'</b> is NOT available. A <b>'//red//'SUBJECT'//black//'</b> is NOT assigned to a section.', &
        '<br>"Enlist" will add the student ONLY to the open sections of a block.', &
        '<br>Seats in an open section may be taken while you are reading this.', &
        '<hr>'
