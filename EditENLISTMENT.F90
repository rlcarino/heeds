...
!    Preenlisted, Section, currentYear, currentTerm)
    if (isDirtyFORM5) call xml_write_pre_enlistment(trim(pathToCurrent)//'ENLISTMENT', Preenlisted, Section)
  ...
  subroutine enlistment_edit (device, NumSections, Section, NumBlocks, Block)
    ...
    integer ::  fdx, mdx, crse, lect, sect, ierr, tLen1, tLen2, pos, n_opts, idx_opt !, bdx, blk,
    ...
    if (isDirtyFORM5) then
        call xml_write_pre_enlistment(trim(pathToCurrent)//'ENLISTMENT', Preenlisted, Section)
!            call write_all_enlistment(trim(dirRAW)//trim(path)//'ENLISTMENT-'//CurrProgCode(targetCurriculum), &
!              Preenlisted, Section, currentYear, currentTerm, targetCurriculum)
!            call write_all_enlistment(trim(dirRAW)//trim(path)//'ENLISTMENT', Preenlisted, Section, currentYear, currentTerm)
      end if
