    ...
    !
    ! this routine needs to be customized with the advising rules of the university
    !
    ...
    subroutine analyze_checklist (std, UseClasses, Offering, WaiverCOI, stdScholastic, &
        ...
        integer, dimension(0:MAX_SUBJECTS_IN_CURRICULUM,0:MAX_ALL_SUBJECT_PREREQ) :: CLPreq
        ...
        character (len=MAX_LEN_SUBJECT_CODE) :: tSubject, token!, tNote
...
!                ! ignore NSTP 1 plans, since NSTP 1 is now required & NSTP 2 is elective
!                if (token(:6) == 'NSTP 1') cycle
#ifdef DBsubst
        ...
        do idx=1,CheckList%NSubjects
            ...
            if (k < 10) then
                CLExt(idx)%Disp_Units = DECDIGITS(k+1:k+1)
            else
                k = mod(k,10)
                CLExt(idx)%Disp_Units = '1'//DECDIGITS(k+1:k+1)
            end if
...
!                ! Only one 190/200/200a must be present
!                do idx=1,n
!                    if (p(idx) == 0) cycle
!                    tSubject = Subject(CheckList%SubjectIdx(p(idx)))%Name
!                    j = index(tSubject, ' 200') + index(tSubject, ' 190')
!                    if (j > 0) then
!                        j = j+3
!                        do i=idx+1,n
!                            if (p(i) == 0) cycle
!                            token = Subject(CheckList%SubjectIdx(p(i)))%Name
!                            if (token(:j) == tSubject(:j)) then
!                                p(i) = 0
!                            end if
!                        end do
!                    end if
