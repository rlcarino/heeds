...
module BLOCKS
    ...
    integer, private :: unitNUMBER=2, eof, ndels, pos(30)
...
contains
    ...
    subroutine xml_write_blocks(path, NumBlocks, Block, Section, iDept)
        ...
        call xml_open_file(unitNUMBER, XML_ROOT_BLOCKS, fileName, i)
        write(unitNUMBER,AFORMAT) &
        '    <comment>', &
        ...
        do blk=1,NumBlocks
            ...
            ! block info start
            call xml_write_character(unitNUMBER, indent0, 'Block')
            call xml_write_character(unitNUMBER, indent1, 'Code', Block(blk)%BlockID)
            call xml_write_character(unitNUMBER, indent1, 'Description', Block(blk)%Name)
            call xml_write_character(unitNUMBER, indent1, 'Curriculum', Curriculum(Block(blk)%CurriculumIdx)%Code)
            call xml_write_integer(unitNUMBER,   indent1, 'Year', Block(blk)%Year)
            call xml_write_integer(unitNUMBER,   indent1, 'Term', Block(blk)%Term)
            call xml_write_character(unitNUMBER, indent1, 'Owner', Department(Block(blk)%DeptIdx)%Code)

            ! write classes assigned to block
            do i=1,Block(blk)%NumClasses
                sect = Block(blk)%Section(i)
                if (sect==0) then
                    call xml_write_character(unitNUMBER, indent2, 'Subject', Subject(Block(blk)%Subject(i))%Name)
                else
                    call xml_write_character(unitNUMBER, indent2, 'Section', Section(sect)%ClassId)
                end if
            end do
        ...
        call xml_close_file(unitNUMBER, XML_ROOT_BLOCKS)
        return
    ...
    subroutine read_blocks(path, NumBlocks, Block, NumSections, Section, errNo)
        ...
        do ddx=2,NumDepartments-1
            fileName = trim(dirXML)//'UPDATES'//DIRSEP//trim(path)//'BLOCKS-'//trim(Department(ddx)%Code)//'.XML'
    ...
    subroutine xml_read_blocks(fName, NumBlocks, Block, NumSections, Section, errNo, openQuietly)
        ...
        call xml_open_file(unitNUMBER, XML_ROOT_BLOCKS, fName, errNo, forReading, quiet)
        if (errNo/=0) return

        ! examine the file line by line
        do
            read(unitNUMBER, AFORMAT, iostat=eof) line
            if (eof<0) exit
        ...
        call xml_close_file(unitNUMBER)
    ...
    subroutine custom_read_blocks(fName, NumBlocks, Block, NumSections, Section, errNo)
        ...
        open(unit=unitNUMBER, file=fName, status='old', iostat=errNo)
        if (errNo/=0) return
