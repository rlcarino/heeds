...
module CURRICULA
    ...
    integer, private :: unitNo=2, eof, ndels, pos(30)
...
#include "custom_read_curricula.F90"
    ...
    subroutine xml_write_curricula(path)
        ...
        do idxCURR=1,NumCurricula
            ...
            call xml_write_character(unitNo, indent0, 'Curriculum')
            call xml_write_character(unitNo, indent1, 'Code', tCurriculum)
            call xml_write_character(unitNo, indent1, 'Title', Curriculum(idxCURR)%Title)
            ...
            call xml_write_character(unitNo, indent1, 'Specialization', mesg)
            ...
            call xml_write_character(unitNo, indent1, 'Remark', mesg)
            ...
            call xml_write_character(unitNo, indent1, 'Status', mesg)

            call xml_write_character(unitNo, indent1, 'College', College(idxCOLL)%Code)
            ...
            do tdx=1,Curriculum(idxCURR)%NumTerms
                ...
                if (mesg==SPACE) cycle
                call xml_write_character(unitNo, indent1, 'Load')
                call xml_write_character(unitNo, indent2, 'Year', txtYear(Year))
                call xml_write_character(unitNo, indent2, 'Term', txtSemester(Term))
                call xml_write_character(unitNo, indent2, 'Subjects', mesg(2:))
                call xml_write_character(unitNo, indent1, '/Load')
            end do
            do tdx=1,NumSubst
        ...
        call xml_close_file(unitNo, XML_ROOT_CURRICULA)
    ...
    subroutine xml_read_curricula(path, errNo)
        ...
        fileName = trim(dirXML)//trim(path)//'CURRICULA.XML'
        call xml_open_file(unitNo, XML_ROOT_CURRICULA, fileName, errNo, forReading)
        if (errNo/=0) return
    ...
    subroutine xml_write_equivalencies(path)
        ...
        call xml_close_file(unitNo, XML_ROOT_EQUIVALENCIES)
    ...
    subroutine xml_read_equivalencies(path, errNo)
        ...
        fileName = trim(dirXML)//trim(path)//'EQUIVALENCIES.XML'
        call xml_open_file(unitNo, XML_ROOT_EQUIVALENCIES, fileName, errNo, forReading)
        if (errNo/=0) return
...
!        end do
    ...
    subroutine xml_write_intake(path)
        ...
        do idxCURR=1,NumCurricula
            ...
            call xml_write_character(unitNo, indent0, 'Intake', &
                trim(Curriculum(idxCURR)%Code)//COMMA//itoa(NFintake(idxCURR)) )
        end do

        call xml_close_file(unitNo, XML_INTAKE)
    ...
    subroutine xml_read_intake(path, errNo)
        ...
        ! open file, return on any error
        fileName = trim(dirXML)//trim(path)//'INTAKE.XML'
        call xml_open_file(unitNo, XML_INTAKE, fileName, errNo, forReading)
        if (errNo/=0) return
