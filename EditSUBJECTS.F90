...
contains
    ...
    subroutine subject_list_all (device)
        ...
        if (nSubjects>0) then
            ...
            do idx=1,nSubjects
                ...
                beginth//'Lect hrs'//endth//beginth//'Min Size'//endth//beginth//'Max Size'//endth,  &
                beginth//'Lab hrs'//endth//beginth//'Min Size'//endth//beginth//'Max Size'//endth//endtr, &
                ...
                begintr//'<td colspan="11"><b>Title:</b> '//trim(Subject(crse)%Title)//endtd//endtr, &
                begintr//'<td colspan="11"><b>Preq.</b> '//trim(text_prerequisite_of_subject(crse,0))//endtd//endtr, &
                begintr//'<td colspan="11">&nbsp;'//endtd//endtr
            end do
  ...
  subroutine subject_edit(device)
    ...
    if (crse/=0 .or. tSubject==SPACE) then
            mesg = 'Subject to edit not specified?'
    else
            crse = index_to_subject(tSubject)
            mesg = 'In subject_edit: Subject code '//tSubject//' is invalid?'
    end if
    if (crse<=0) then ! subject code is invalid
            write(device,AFORMAT) '<br>'//red//trim(mesg)//black//'<hr><br>'
            targetCollege = CollegeIdxUser
            return
    end if
