...
contains
  ...
  subroutine block_show_schedule(device, NumSections, Section, Offering, NumBlocks, Block, fn, given)
    ...
    if (present(given)) then
            targetBlock = given
            tBlock = Block(given)%BlockID
            ierr = 0
    else
            call cgi_get_named_string(QUERY_STRING, 'A1', tBlock, ierr)
            targetBlock = index_to_block(tBlock, NumBlocks, Block)
            if (ierr/=0 .or. targetBlock==0) then
                    targetDepartment = DeptIdxUser
                    targetCollege = CollegeIdxUser
                    call html_write_header(device, 'Block schedule', '<hr><br>Block "'//tBlock//'" not found')
                    return
            end if
    end if
    ...
    select case (fn)
        ...
        case (fnBlockCopy, fnNextBlockCopy)
                call cgi_get_named_string(QUERY_STRING, 'BlockID', newBlock, ierr)
                if (newBlock==SPACE .or. ierr/=0) then
        ...
        case (fnBlockEditSection, fnNextBlockEditSection)
    ...
    call list_sections_to_edit(device, Section, tLen1, tArray, fnOFFSET+fnBlockEditSection, tBlock, &
  ...
  subroutine block_add(device, Term, NumSections, Section, Offering, NumBlocks, Block)
    ...
    if (ierr/=0 .or. targetCurriculum<=0) then
            ...
            if (tYear=='ALL') then
                    YearFirst = 1
                    YearLast = (Curriculum(targetCurriculum)%NumTerms+1)/3
            else
