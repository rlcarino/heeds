...
!1       2      3     4    5       6
    ...
    subroutine write_student_info_csv()

        integer :: ierr, std, idx

        fileName = trim(dirSTUDENTINFO)//trim(UniversityCode)//DASH//'STUDENT-INFO.CSV'
        write(*,*) 'Writing '//trim(fileName)

        open(unit=unitETC, file=fileName)
        ...
        do idx=1,NumStudents+NumAdditionalStudents
            ...
            write(unitETC,AFORMAT) trim(line)
        end do
        close(unitETC)
