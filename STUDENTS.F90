...
module STUDENTS
    ...
    type :: TYPE_STUDENT
        ...
        integer :: Reqd(-1:5,0:MAX_SUBJECTS_IN_CURRICULUM/3)  ! Plan of study: indices to required subjects
        integer :: Subst(0:5,0:MAX_SUBJECTS_IN_CURRICULUM/3) ! Plan of study: indices to replacement subjects
...
#include "custom_read_students.F90"
    ...
    subroutine collect_prefix_years()
        ...
        do std=1,NumStudents
            ...
            jdx = index(StdNoPrefix, ':'//Student(std)%StdNo(:idx))
    ...
    subroutine xml_write_students(path, iCurr)
        ...
        ! write file
        call xml_open_file(unitXML, XML_ROOT_STUDENTS, fileName, std)
    ...
    subroutine xml_read_students(path, iCurr, numEntries, errNo)
        ...
        numEntries = 0
        call xml_open_file(unitXML, XML_ROOT_STUDENTS, fileName, errNo, forReading)
        if (errNo/=0) return
