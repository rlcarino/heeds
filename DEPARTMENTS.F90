...
#include "custom_read_departments.F90"
    ...
    subroutine xml_write_departments(path)
        ...
        do ldx = 2,NumDepartments-1
            call xml_write_character(unitNum, indent0, 'Department')
            call xml_write_character(unitNum, indent1, 'Code', Department(ldx)%Code)
            call xml_write_character(unitNum, indent1, 'Name', Department(ldx)%Name)
            call xml_write_character(unitNum, indent1, 'College', College(Department(ldx)%CollegeIdx)%Code)
            if (Department(ldx)%SectionPrefix/=SPACE) &
                call xml_write_character(unitNum, indent1, 'SectionPrefix', Department(ldx)%SectionPrefix)
            call xml_write_character(unitNum, indent0, '/Department')
        end do

        call xml_close_file(unitNum, XML_ROOT_DEPARTMENTS)
    ...
    subroutine xml_read_departments(path, errNo)
        ...
        fileName = trim(dirXML)//trim(path)//'DEPARTMENTS.XML'
        call xml_open_file(unitNum, XML_ROOT_DEPARTMENTS, fileName, errNo, forReading)
        if (errNo/=0) return
