...
!        call xml_open_file(unitETC, XML_ROOT_TEACHERS, fileName, ldx)
!
...
!        do ldx = 1,NumTeachers+NumAdditionalTeachers
!
!            call xml_write_character(unitETC, indent0, 'Teacher')
!            call xml_write_character(unitETC, indent1, 'TeacherId', Teacher(ldx)%TeacherId)
!            if (Teacher(ldx)%MaxLoad>0) &
!                call xml_write_integer(unitETC, indent1, 'MaxLoad', Teacher(ldx)%MaxLoad)
!            if (Teacher(ldx)%Rank>0) &
!                call xml_write_character(unitETC, indent1, 'Rank', AcademicRank(Teacher(ldx)%Rank))
!            if (Teacher(ldx)%Step>0) &
!                call xml_write_character(unitETC, indent1, 'Step', RankStep(Teacher(ldx)%Step))
!            if (Teacher(ldx)%Bachelor/=SPACE) &
!                call xml_write_character(unitETC, indent1, 'Bachelor', Teacher(ldx)%Bachelor)
!            if (Teacher(ldx)%Master/=SPACE) &
!                call xml_write_character(unitETC, indent1, 'Master', Teacher(ldx)%Master)
!            if (Teacher(ldx)%Doctorate/=SPACE)  &
!                call xml_write_character(unitETC, indent1, 'Doctorate', Teacher(ldx)%Doctorate)
!            if (Teacher(ldx)%Specialization/=SPACE)  &
!                call xml_write_character(unitETC, indent1, 'Specialization', Teacher(ldx)%Specialization)
!            if (Teacher(ldx)%Password/=SPACE)  &
!                call xml_write_character(unitETC, indent1, 'Password', Teacher(ldx)%Password)
!            if (Teacher(ldx)%Role/=SPACE)  &
!                call xml_write_character(unitETC, indent1, 'Role', Teacher(ldx)%Role)
!            call xml_write_character(unitETC, indent0, '/Teacher')
!
!        end do
!
!        call xml_close_file(unitETC, XML_ROOT_TEACHERS)
